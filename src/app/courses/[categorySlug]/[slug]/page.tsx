...
export async function generateMetadata({ params }: { params: { categorySlug: string; slug: string } }): Promise<Metadata> {
  const course = await getCourse(params.categorySlug, params.slug);
...
export default async function NestedCourseDetailPage({ params }: { params: { categorySlug: string; slug: string } }) {
  const course = await getCourse(params.categorySlug, params.slug);
  if (!course) notFound();
      ...
      <div style={{ background: 'linear-gradient(135deg, #1a1a2e, #0f3460)', padding: '48px 20px 40px', color: '#fff' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <div style={{ marginBottom: '12px' }}>
            ...
            <Link href={`/courses/${params.categorySlug}`} style={{ color: 'rgba(255,255,255,0.65)', fontSize: '13px' }}>{course.category}</Link>
            <span style={{ color: 'rgba(255,255,255,0.4)', margin: '0 8px' }}>›</span>
