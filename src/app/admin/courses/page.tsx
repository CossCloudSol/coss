  ...
  return (
    <div className="space-y-5">
      ...
      <div className="block lg:hidden">
        {loading ? loader : courses.length === 0 ? empty : (
          <div className="flex flex-col gap-2">
            {courses.map((course, index) => {
              ...
              const modeBadgeCls = isPublished
                ? 'bg-white/20 text-white'
                : 'bg-[#1d4ed8] text-white dark:bg-[#1a2c3d] dark:text-[#58a6ff]';
              const levelBadgeCls = isPublished
                ? 'bg-white/20 text-white'
                : 'bg-[#d97706] text-white';

              return (
                <div key={course.id} className={`rounded-xl p-3 ${cardBg}`}>
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <p className={`text-sm leading-snug ${titleCls}`}>{course.title}</p>
                    <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium capitalize ${statusBadgeCls}`}>
                      {course.status}
                    </span>
                  </div>
                  <p className={`text-xs mb-2 ${subtitleCls}`}>{course.category} · {course.duration}</p>
                  <div className="flex gap-1.5 mb-3 flex-wrap">
                    <span className={`rounded-full px-2 py-0.5 text-xs ${modeBadgeCls}`}>{course.mode}</span>
                    <span className={`rounded-full px-2 py-0.5 text-xs ${levelBadgeCls}`}>{course.level}</span>
                  </div>
