...
export default function EditTrainerPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/trainers/${params.id}`)
      .then((r) => r.json())
      .then((trainer) => {
        setForm({
          name: trainer.name ?? '',
          ...
          startYear: trainer.startYear?.toString() ?? '',
          bio: trainer.bio ?? '',
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/trainers/${params.id}`, {
        method: 'PUT',
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Edit Trainer</h1>
      </div>
