...
import { Plus, Edit2, Trash2, Lock, Loader2, FolderOpen, ChevronRight } from 'lucide-react';
    ...
    <div className="space-y-5 overflow-x-hidden">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {!loading && (
            <span className="bg-[#e6f4f6] text-[#024c57] border border-[#c9e8ed] rounded-full px-2 py-0.5 text-sm dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600">
              {categories.length}
            </span>
          )}
        </div>
        <Link
          ...
          className="inline-flex items-center gap-2 bg-[#024c57] text-white hover:bg-[#03798a] rounded-xl px-4 py-2 font-medium text-sm transition-colors"
        >
          ...
          <div className="block lg:hidden space-y-2">
            {categories.map((cat) => (
              <Link
                ...
                className="flex items-center gap-2 bg-white dark:bg-[#161b22] border border-[#c9e8ed] dark:border-[#21262d] rounded-xl p-3"
              >
                {cat.color && (
                  <span
                ...
                <span className="text-[#0f172a] dark:text-[#e6edf3] font-medium text-sm flex-1 min-w-0 truncate">{cat.name}</span>
                {cat.isLegacy && (
                  <span className="bg-[#fef3c7] text-[#92400e] dark:bg-[#2d1f0a] dark:text-[#d29922] text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0">
                    Legacy
                  </span>
                )}
                <div className="flex items-center gap-1 ml-auto flex-shrink-0">
                  {cat.isLegacy && <Lock className="h-3 w-3 text-[#94a3b8] dark:text-[#8b949e]" />}
                  <span className="text-[#03798a] dark:text-[#8b949e] text-xs font-mono truncate max-w-[100px]">{cat.slug}</span>
                  <ChevronRight className="h-4 w-4 text-[#03798a] dark:text-[#8b949e] flex-shrink-0" />
                </div>
              </Link>
            ))}
          </div>
