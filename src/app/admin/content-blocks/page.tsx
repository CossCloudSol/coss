...
const BLOCK_TYPE_COLORS: Record<string, string> = {
  hero:         'bg-indigo-50 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border-indigo-200 dark:border-indigo-500/30',
  feature:      'bg-teal-50 dark:bg-teal-500/20 text-teal-700 dark:text-teal-300 border-teal-200 dark:border-teal-500/30',
  faq:          'bg-amber-50 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-500/30',
  timeline:     'bg-purple-50 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 border-purple-200 dark:border-purple-500/30',
  'course-grid':'bg-blue-50 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-500/30',
  'blog-grid':  'bg-green-50 dark:bg-green-500/20 text-green-700 dark:text-green-300 border-green-200 dark:border-green-500/30',
  testimonials: 'bg-pink-50 dark:bg-pink-500/20 text-pink-700 dark:text-pink-300 border-pink-200 dark:border-pink-500/30',
  cta:          'bg-orange-50 dark:bg-orange-500/20 text-orange-700 dark:text-orange-300 border-orange-200 dark:border-orange-500/30',
}
...
function GridStyleSelector({ value, onChange }: { value: GridStyle; onChange: (v: GridStyle) => void }) {
  return (
    <div>
      <label className="text-xs text-[#475569] dark:text-gray-400 uppercase tracking-wide font-medium block mb-2">Grid style</label>
        ...
        {GRID_STYLES.map(style => (
          <button
            ...
            className={`flex items-center gap-3 p-2.5 rounded-lg border text-left transition-colors ${
              value === style.id
                ? 'border-teal-500/50 bg-teal-50 dark:bg-teal-500/5'
                : 'border-[#e2e8f0] dark:border-white/10 bg-transparent hover:bg-gray-100 dark:hover:bg-gray-700'
            }`}
          >
            {/* Mini preview thumbnail */}
            <div className={`w-14 h-10 rounded flex-shrink-0 overflow-hidden border ${
              value === style.id ? 'border-teal-500/30' : 'border-[#e2e8f0] dark:border-white/10'
            }`}>
              {style.id === 'card-stack' && (
                <div className="w-full h-full bg-gray-200 dark:bg-gray-700 grid grid-cols-3 gap-0.5 p-0.5">
                  {[0, 1, 2].map(i => (
                    <div key={i} className="bg-gray-300 dark:bg-gray-600 rounded-sm flex flex-col">
                      <div className="h-1/2 bg-gray-400 dark:bg-gray-500 rounded-t-sm" />
                    </div>
                  ))}
                </div>
              )}
              {style.id === 'horizontal' && (
                <div className="w-full h-full bg-gray-200 dark:bg-gray-700 flex flex-col gap-0.5 p-0.5">
                  {[0, 1, 2].map(i => (
                    <div key={i} className="flex-1 bg-gray-300 dark:bg-gray-600 rounded-sm flex gap-0.5">
                      <div className="w-1/3 bg-gray-400 dark:bg-gray-500 rounded-l-sm" />
                    </div>
                  ))}
                </div>
              )}
              {style.id === 'image-hero' && (
                <div className="w-full h-full bg-gray-200 dark:bg-gray-700 grid grid-cols-3 gap-0.5 p-0.5">
                  {[0, 1, 2].map(i => (
                    <div key={i} className="bg-gray-400 dark:bg-gray-500 rounded-sm relative overflow-hidden">
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-xs font-medium ${value === style.id ? 'text-teal-600 dark:text-teal-400' : 'text-[#0f172a] dark:text-gray-200'}`}>
                {style.label}
              </p>
              <p className="text-xs text-[#94a3b8] dark:text-gray-500 mt-0.5 leading-tight">{style.desc}</p>
            </div>
            {value === style.id && (
...
function BlockPreview({ block }: { block: ContentBlock }) {
  ...
  if (t === 'hero') return (
    <div className="bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 rounded-lg p-4 text-center">
      <div className="text-lg font-bold text-gray-900 dark:text-white mb-1">{block.title || 'Hero Heading'}</div>
      <div className="text-xs text-[#475569] dark:text-gray-400 mb-3">{block.body || 'Subheading text goes here'}</div>
      <div className="flex gap-2 justify-center">
        <span className="px-3 py-1 text-xs bg-teal-600 text-white rounded">Primary CTA</span>
        <span className="px-3 py-1 text-xs border border-gray-300 dark:border-white/20 text-[#475569] dark:text-gray-300 rounded">Secondary</span>
      </div>
    </div>
  )
  if (t === 'feature') return (
    <div className="grid grid-cols-3 gap-2">
      {[0, 1, 2].map(i => (
        <div key={i} className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-2 text-center">
          <div className="w-6 h-6 bg-teal-500/20 rounded mx-auto mb-1" />
          <div className="text-xs text-[#475569] dark:text-gray-300">Feature {i + 1}</div>
        </div>
      ))}
    </div>
  )
  if (t === 'faq') return (
    <div className="space-y-1.5">
      {[block.title || 'Question 1', 'Question 2', 'Question 3'].map((q, i) => (
        <div key={i} className="bg-gray-100 dark:bg-gray-700/50 rounded px-3 py-2 flex items-center justify-between">
          <span className="text-xs text-[#475569] dark:text-gray-300 truncate">{q}</span>
          <span className="text-[#94a3b8] dark:text-gray-500 text-xs ml-2">▼</span>
        </div>
      ))}
    </div>
  )
    ...
    <div className="relative pl-4">
      ...
      {[block.title || '2010', '2016', '2020'].map((yr, i) => (
        <div key={i} className="relative mb-2 pl-3">
          ...
          <span className="text-xs font-bold text-teal-600 dark:text-teal-400">{yr}</span>
          <p className="text-xs text-[#64748b] dark:text-gray-400">{i === 0 ? (block.body?.slice(0, 40) || 'Event text') : `Event ${i + 1}`}</p>
        </div>
      ))}
    </div>
  )
  if (t === 'course-grid') {
    ...
    if (gs === 'horizontal') return (
      <div className="space-y-1.5">
        {[0, 1, 2].map(i => (
          <div key={i} className="flex gap-2 bg-gray-100 dark:bg-gray-700/50 rounded p-1.5">
            <div className="w-12 h-8 bg-gray-300 dark:bg-gray-600 rounded flex-shrink-0" />
            <div className="flex-1"><div className="text-xs text-[#475569] dark:text-gray-300">Course title</div><div className="text-xs text-[#94a3b8] dark:text-gray-500">Category</div></div>
          </div>
        ))}
      </div>
    )
    if (gs === 'image-hero') return (
      <div className="grid grid-cols-3 gap-1.5">
        {[0, 1, 2].map(i => (
          <div key={i} className="bg-gray-400 dark:bg-gray-600 rounded h-14 relative overflow-hidden">
    ...
    return (
      <div className="grid grid-cols-3 gap-1.5">
        {[0, 1, 2].map(i => (
          <div key={i} className="bg-gray-100 dark:bg-gray-700/50 rounded overflow-hidden">
            <div className="h-8 bg-gray-300 dark:bg-gray-600" />
            <div className="p-1.5"><div className="text-xs text-[#475569] dark:text-gray-300">Course {i + 1}</div></div>
          </div>
        ))}
      </div>
    )
    ...
    if (gs === 'horizontal') return (
      <div className="space-y-1.5">
        {[0, 1, 2].map(i => (
          <div key={i} className="flex gap-2 bg-gray-100 dark:bg-gray-700/50 rounded p-1.5">
            <div className="w-12 h-8 bg-gray-300 dark:bg-gray-600 rounded flex-shrink-0" />
            <div className="flex-1"><div className="text-xs text-[#475569] dark:text-gray-300">Blog post title</div><div className="text-xs text-[#94a3b8] dark:text-gray-500">5 min read</div></div>
          </div>
        ))}
      </div>
    )
    ...
    return (
      <div className="grid grid-cols-3 gap-1.5">
        {[0, 1, 2].map(i => (
          <div key={i} className="bg-gray-100 dark:bg-gray-700/50 rounded overflow-hidden">
            <div className="h-8 bg-gray-300 dark:bg-gray-600" />
            <div className="p-1.5"><div className="text-xs text-[#475569] dark:text-gray-300">Post {i + 1}</div></div>
          </div>
        ))}
      </div>
    )
  ...
  if (t === 'testimonials') return (
    <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3">
      <div className="flex text-yellow-500 dark:text-yellow-400 text-xs mb-1">★★★★★</div>
      <p className="text-xs text-[#475569] dark:text-gray-300 italic">&quot;{block.body?.slice(0, 60) || 'Student testimonial quote goes here...'}&quot;</p>
      <p className="text-xs text-[#94a3b8] dark:text-gray-500 mt-1">— {block.title || 'Student Name'}</p>
    </div>
  )
  if (t === 'cta') return (
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-700/30 rounded p-3 text-xs text-[#64748b] dark:text-gray-400 text-center">
...
function SettingsPanel({
  ...
  const colorClass = BLOCK_TYPE_COLORS[block.blockType] ?? 'bg-gray-100 dark:bg-gray-500/20 text-[#475569] dark:text-gray-300 border-gray-200 dark:border-gray-500/30'

  const inputCls = 'w-full bg-gray-50 dark:bg-gray-700 border border-[#e2e8f0] dark:border-white/10 rounded-md px-3 py-2 text-sm text-[#0f172a] dark:text-white placeholder-[#94a3b8] dark:placeholder-gray-500 focus:outline-none focus:border-teal-500/50'
  const labelCls = 'block text-xs text-[#475569] dark:text-gray-400 mb-1'
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-[#e2e8f0] dark:border-white/10">
        ...
        <p className="text-xs text-[#64748b] dark:text-gray-400 mt-1.5 truncate">{block.title || '(untitled)'}</p>
      </div>

      {/* Tabs */}
      <div className="flex border-b border-[#e2e8f0] dark:border-white/10">
          ...
          <button
            ...
            className={`flex-1 py-2 text-xs font-medium capitalize transition-colors ${
              tab === t
                ...
                : 'text-[#94a3b8] dark:text-gray-500 hover:text-[#475569] dark:hover:text-gray-300'
            }`}
          >
            {t}
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <>
            ...
            {block.blockType === 'faq' && (
              <>
                ...
                <div className="pt-2 border-t border-[#e2e8f0] dark:border-white/10">
                  <p className="text-xs text-[#94a3b8] dark:text-gray-500 mb-2">All FAQs on this page</p>
                  <div className="space-y-1 max-h-40 overflow-y-auto">
                    {allBlocks.filter(b => b.blockType === 'faq').map(b => (
                      <div key={b.id} className={`px-2 py-1.5 rounded text-xs truncate ${b.id === block.id ? 'bg-teal-50 dark:bg-teal-500/10 text-teal-700 dark:text-teal-300' : 'text-[#64748b] dark:text-gray-400 bg-gray-100 dark:bg-gray-700/30'}`}>
                        {b.title || '(untitled)'}
                      </div>
                    ))}
                  </div>
            ...
            {block.blockType === 'timeline' && (
              <>
                ...
                <div className="pt-2 border-t border-[#e2e8f0] dark:border-white/10">
                  <p className="text-xs text-[#94a3b8] dark:text-gray-500 mb-2">All timeline entries</p>
                  <div className="space-y-1 max-h-40 overflow-y-auto">
              ...
              <>
                ...
                {block.blockType === 'course-grid' && (
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 text-xs text-[#475569] dark:text-gray-300 cursor-pointer">
        ...
        {tab === 'advanced' && (
          <>
            ...
            <button
              onClick={onDelete}
              className="w-full mt-2 px-3 py-2 text-xs bg-[#fef2f2] dark:bg-transparent text-[#b91c1c] dark:text-red-400 border border-[#fecaca] dark:border-red-500/30 rounded-lg hover:bg-[#fee2e2] dark:hover:bg-red-500/10 transition-colors"
            >
              Delete block
            </button>
          </>
      ...
      {/* Save button */}
      <div className="p-4 border-t border-[#e2e8f0] dark:border-white/10">
        <button
          onClick={onSave}
          disabled={saving}
...
export default function ContentBlocksPage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-white dark:bg-gray-900 text-[#0f172a] dark:text-white overflow-hidden">
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 right-4 z-50 bg-white dark:bg-gray-800 border border-[#e2e8f0] dark:border-white/10 text-[#0f172a] dark:text-white text-sm px-4 py-2 rounded-lg shadow-lg">
          {toast}
        </div>
      )}

      {/* ── Topbar ── */}
      <header className="flex items-center justify-between px-5 py-3 border-b border-[#e2e8f0] dark:border-white/10 bg-white dark:bg-gray-900 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-sm font-semibold text-[#0f172a] dark:text-white">Content blocks editor</span>
            <span className="ml-2 text-xs bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded text-[#475569] dark:text-gray-300">Coss IMS</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {draftChanges && <span className="text-xs text-amber-600 dark:text-amber-400">● Unsaved changes</span>}
          <button
      ...
      <div className="hidden lg:flex flex-1 overflow-hidden">
        {/* Left sidebar */}
        <aside className="w-[220px] flex-shrink-0 border-r border-[#e2e8f0] dark:border-white/10 bg-white dark:bg-gray-900 flex flex-col overflow-hidden">
          {/* Pages */}
          <div className="p-3 border-b border-[#e2e8f0] dark:border-white/10">
            <p className="text-xs text-[#94a3b8] dark:text-gray-500 uppercase tracking-wide font-medium mb-2">Pages</p>
                ...
                <li key={p.id}>
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-left transition-colors ${
                      activePage === p.id
                        ? 'bg-teal-50 dark:bg-teal-600/20 text-teal-700 dark:text-teal-300'
                        : 'text-[#475569] dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-[#0f172a] dark:hover:text-white'
                    }`}
                  >
                    <span>{p.emoji}</span>
                    <span>{p.label}</span>
                    {activePage === p.id && (
                      <span className="ml-auto text-xs text-[#94a3b8] dark:text-gray-500">{blocks.length}</span>
                    )}
                  </button>
                </li>
              ))}
          ...
          {/* Block type library */}
          <div className="p-3 border-b border-[#e2e8f0] dark:border-white/10 flex-1 overflow-y-auto">
            <p className="text-xs text-[#94a3b8] dark:text-gray-500 uppercase tracking-wide font-medium mb-2">Add block</p>
            <div className="flex flex-col gap-1.5">
              {BLOCK_TYPES.map(bt => (
                <button
                  key={bt.type}
                  onClick={() => handleAddBlock(bt.type)}
                  className="flex items-center gap-2 px-3 py-2 rounded-lg border border-[#e2e8f0] dark:border-white/10 text-xs text-[#475569] dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-[#0f172a] dark:hover:text-white transition-colors text-left"
                >
                  <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                    BLOCK_TYPE_COLORS[bt.type]?.includes('teal') ? 'bg-teal-500' :
                    BLOCK_TYPE_COLORS[bt.type]?.includes('indigo') ? 'bg-indigo-500' :
                    BLOCK_TYPE_COLORS[bt.type]?.includes('amber') ? 'bg-amber-500' :
                    BLOCK_TYPE_COLORS[bt.type]?.includes('purple') ? 'bg-purple-500' :
                    BLOCK_TYPE_COLORS[bt.type]?.includes('blue') ? 'bg-blue-500' :
                    BLOCK_TYPE_COLORS[bt.type]?.includes('green') ? 'bg-green-500' :
                    BLOCK_TYPE_COLORS[bt.type]?.includes('pink') ? 'bg-pink-500' : 'bg-orange-500'
                  }`} />
                  + {bt.label}
                </button>
              ))}
          ...
          {/* Stats */}
          <div className="p-3">
            <p className="text-xs text-[#94a3b8] dark:text-gray-500 uppercase tracking-wide font-medium mb-2">Stats</p>
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-[#94a3b8] dark:text-gray-500">Total blocks</span>
                <span className="text-[#475569] dark:text-gray-300">{blocks.length}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-[#94a3b8] dark:text-gray-500">Visible</span>
                <span className="text-teal-600 dark:text-teal-400">{blocks.filter(b => b.isVisible).length}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-[#94a3b8] dark:text-gray-500">Hidden</span>
                <span className="text-[#94a3b8] dark:text-gray-500">{blocks.filter(b => !b.isVisible).length}</span>
              </div>
            </div>
          </div>
        </aside>
        ...
        {/* Center canvas */}
        <main className="flex-1 overflow-y-auto p-5 bg-[#f0f4f8] dark:bg-gray-950">
          {/* Page header */}
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-base font-semibold text-[#0f172a] dark:text-white capitalize">
                {PAGES.find(p => p.id === activePage)?.label ?? activePage} blocks
              </h1>
              <p className="text-xs text-[#94a3b8] dark:text-gray-500 mt-0.5">{blocks.length} block{blocks.length !== 1 ? 's' : ''}</p>
            </div>
          </div>

          {loading ? (
            <div className="flex items-center justify-center py-16 text-[#94a3b8] dark:text-gray-500 text-sm">Loading blocks...</div>
          ) : blocks.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-12 h-12 bg-gray-100 dark:bg-gray-800 rounded-xl mb-4 flex items-center justify-center text-2xl">📦</div>
              <p className="text-[#475569] dark:text-gray-400 text-sm">No blocks on this page</p>
              <p className="text-[#94a3b8] dark:text-gray-600 text-xs mt-1">Click a block type in the sidebar to add one</p>
            </div>
          ) : (
                ...
                return (
                  <div
                    ...
                    className={`group rounded-xl border cursor-pointer transition-all ${
                      isSelected
                        ? 'border-teal-500/50 bg-white dark:bg-gray-800 shadow-lg shadow-teal-500/5'
                        : 'border-[#e2e8f0] dark:border-white/8 bg-white dark:bg-gray-900 hover:border-gray-300 dark:hover:border-white/20 hover:bg-gray-50 dark:hover:bg-gray-800'
                    } ${!block.isVisible ? 'opacity-50' : ''}`}
                  >
                    {/* Card toolbar */}
                    <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#f1f5f9] dark:border-white/8">
                      <div className="flex items-center gap-2">
                        <span className={`text-xs px-2 py-0.5 rounded border font-medium ${colorClass}`}>
                          {block.blockType}
                        </span>
                        <span className="text-xs text-[#64748b] dark:text-gray-400 truncate max-w-[180px]">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          disabled={index === 0}
                          className="p-1 rounded text-[#94a3b8] dark:text-gray-600 hover:text-[#475569] dark:hover:text-gray-300 disabled:opacity-20 transition-colors"
                          title="Move up"
                        >
                        ...
                        <button
                          ...
                          className={`p-1 rounded transition-colors ${block.isVisible ? 'text-teal-600 dark:text-teal-400 hover:text-teal-500 dark:hover:text-teal-300' : 'text-[#94a3b8] dark:text-gray-600 hover:text-[#64748b] dark:hover:text-gray-400'}`}
                        ...
                        <button
                          ...
                          className="p-1 rounded text-[#94a3b8] dark:text-gray-600 hover:text-[#b91c1c] dark:hover:text-red-400 hover:bg-[#fef2f2] dark:hover:bg-red-500/10 transition-colors"
                          title="Delete block"
                        >
                        ...
                        <span className="text-xs text-[#94a3b8] dark:text-gray-600 ml-1">#{index + 1}</span>
                      </div>
                    </div>
        ...
        <aside className="w-[280px] flex-shrink-0 border-l border-[#e2e8f0] dark:border-white/10 bg-white dark:bg-gray-900 flex flex-col overflow-hidden">
          {selectedBlock ? (
            <SettingsPanel
              block={selectedBlock}
              tab={activeTab}
            ...
            <div className="flex flex-col items-center justify-center h-full text-center px-6">
              <div className="w-10 h-10 bg-gray-100 dark:bg-gray-800 rounded-xl mb-3 flex items-center justify-center">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-[#94a3b8] dark:text-gray-600">
                  <path d="M12 20h9" /><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z" />
                </svg>
              </div>
              <p className="text-sm text-[#475569] dark:text-gray-500">Select a block</p>
              <p className="text-xs text-[#94a3b8] dark:text-gray-600 mt-1">Click any block on the canvas to edit its settings</p>
            </div>
          )}
        </aside>
      </div>
      ...
      <div className="flex lg:hidden flex-col flex-1 overflow-hidden overflow-x-hidden">
        ...
        <div className="p-3 border-b border-[#e2e8f0] dark:border-white/10 flex-shrink-0">
          <select
            value={activePage}
            onChange={e => setActivePage(e.target.value)}
            className="w-full bg-gray-50 dark:bg-gray-800 border border-[#e2e8f0] dark:border-white/10 rounded-lg px-3 py-2.5 text-sm text-[#0f172a] dark:text-white focus:outline-none focus:border-teal-500/50 appearance-none"
          >
            {PAGES.map(p => (
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-12 h-12 bg-gray-100 dark:bg-gray-800 rounded-xl mb-4 flex items-center justify-center text-2xl">📦</div>
              <p className="text-[#475569] dark:text-gray-400 text-sm">No blocks on this page</p>
              <p className="text-[#94a3b8] dark:text-gray-600 text-xs mt-1">Tap a block type below to add one</p>
            </div>
          ) : (
            <div className="space-y-2">
              {blocks.map(block => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${
                      selectedId === block.id
                        ? 'border-teal-500/50 bg-white dark:bg-gray-800'
                        : 'border-[#e2e8f0] dark:border-white/8 bg-white dark:bg-gray-900'
                    } ${!block.isVisible ? 'opacity-50' : ''}`}
                  >
                    {/* Drag handle */}
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[#94a3b8] dark:text-gray-600 flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[#475569] dark:text-gray-300 truncate mt-1">{block.title || '(untitled)'}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded transition-colors ${block.isVisible ? 'text-teal-600 dark:text-teal-400' : 'text-[#94a3b8] dark:text-gray-600'}`}
                        title={block.isVisible ? 'Hide block' : 'Show block'}
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          {block.isVisible
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 text-xs text-teal-600 dark:text-teal-400 border border-teal-200 dark:border-teal-500/30 rounded-md hover:bg-teal-50 dark:hover:bg-teal-500/10 transition-colors whitespace-nowrap"
                      >
                        Edit settings
                      </button>
                    </div>
        ...
        {/* Add block pill row */}
        <div className="flex-shrink-0 border-t border-[#e2e8f0] dark:border-white/10 px-3 py-2.5">
          ...
          <div className="pill-scroll flex gap-2 overflow-x-auto pb-0.5" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {BLOCK_TYPES.map(bt => (
              <button
                key={bt.type}
                onClick={() => handleAddBlock(bt.type)}
                className="flex-shrink-0 px-3 py-1.5 text-xs text-[#475569] dark:text-gray-300 border border-[#e2e8f0] dark:border-white/10 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-[#0f172a] dark:hover:text-white transition-colors whitespace-nowrap"
              >
                + {bt.label}
              </button>
            ))}
        ...
        {mounted && mobileSheetOpen && selectedBlock && createPortal(
          <>
            ...
            />
            {/* Sheet */}
            <div className="fixed bottom-0 left-0 right-0 z-50 rounded-t-2xl bg-white dark:bg-gray-900 flex flex-col" style={{ maxHeight: '85vh' }}>
              {/* Sheet header */}
              <div className="flex items-center justify-between px-4 py-3 border-b border-[#e2e8f0] dark:border-white/10 flex-shrink-0">
                <p className="text-sm font-medium text-[#0f172a] dark:text-white">Edit block</p>
                <button
                  onClick={() => setMobileSheetOpen(false)}
                  className="w-7 h-7 flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 text-[#475569] dark:text-gray-400 hover:text-[#0f172a] dark:hover:text-white transition-colors text-lg leading-none"
                  aria-label="Close"
                >
                  ×
                </button>
