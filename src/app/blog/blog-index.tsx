 ...
 *   - src/app/blog/page.tsx                          → /blog (All, page 1)
 *   - src/app/blog/filter/[category]/[page]/page.tsx → /blog?category=…&page=…
 *     (reached through the beforeFiles rewrites in next.config.mjs; the
 *     public URLs keep their query strings)
 * Neither route reads searchParams, so both are prerendered/ISR instead of
...
const getPublishedDbPosts = unstable_cache(
  async () =>
  ...
  ['blog-index-db-posts'],
  { tags: [BLOG_POSTS_TAG], revalidate: 86400 },
);

type BlogIndexData = { dbPosts: DBPost[]; mdxPosts: MDXPost[] };

/** Both post sources, normalised. React cache() dedupes within one render/build worker. */
export const getBlogIndexData = cache(async (): Promise<BlogIndexData> => {
  const [dbPostsRaw, mdxPostsRaw] = await Promise.all([getPublishedDbPosts(), getAllPosts()]);
...
function filterByCategory<T extends { category: string }>(posts: T[], activeCategory: string): T[] {
  return activeCategory === 'All' ? posts : posts.filter(p => matchesCategory(p.category, activeCategory));
}

/** Number of MDX pagination pages for a category (min 1). */
export function blogPageCount(data: BlogIndexData, activeCategory: string): number {
  return Math.max(1, Math.ceil(filterByCategory(data.mdxPosts, activeCategory).length / POSTS_PER_PAGE));
}
