...
export interface WhatsAppClicksAdminResponse {
  stats: {
    total: number;
    thisMonth: number;
    topCtaType: string;
    topPageType: string;
    byCtaType: Array<{ ctaType: string; count: number }>;
    byPageType: Array<{ pageType: string; count: number }>;
  };
  recent: WhatsAppClickRow[];
  // Four independent per-channel series, NEVER summed into one total — a
  // WhatsApp click is an anonymous event, not a contactable lead, and a
  // combined "total leads" figure would overstate real pipeline volume.
  // Mirrors /api/admin/call-clicks's series, extended with the whatsappClick
  // channel this page adds.
  series: {
...
function bucketSeries(
  ...
  count: number,
  channels: { form: Date[]; whatsappLead: Date[]; callClick: Date[]; whatsappClick: Date[] },
): ChannelBucket[] {
  const now = new Date();
  ...
  const buckets = new Map<string, ChannelBucket>(
    labels.map((label) => [label, { label, form: 0, whatsappLead: 0, callClick: 0, whatsappClick: 0 }]),
  );

  const tally = (dates: Date[], field: 'form' | 'whatsappLead' | 'callClick' | 'whatsappClick') => {
  ...
  tally(channels.whatsappLead, 'whatsappLead');
  tally(channels.callClick, 'callClick');
  tally(channels.whatsappClick, 'whatsappClick');
...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  try {
    const [
      ...
      prisma.whatsAppClick.count({ where: { createdAt: { gte: monthStart } } }),
      ...
      prisma.lead.findMany({
        where: { formType: { in: ['hero', 'hero_demo', 'full', 'demo', 'contact'] }, createdAt: { gte: twelveMonthsAgo } },
    ...
    const topCtaType = ctaTypeGroups.length > 0 ? ctaTypeGroups[0].ctaType : '—';
    const topPageType = pageTypeGroups.length > 0 ? pageTypeGroups[0].pageType : '—';

    const channels = {
      form: formLeadDates.map((r) => r.createdAt),
      whatsappLead: whatsappLeadDates.map((r) => r.createdAt),
      callClick: callClickDates.map((r) => r.createdAt),
      whatsappClick: whatsappClickDates.map((r) => r.createdAt),
    };

    const payload: WhatsAppClicksAdminResponse = {
      stats: {
        total,
        thisMonth,
        topCtaType,
        topPageType,
        byCtaType: ctaTypeGroups.map((g) => ({ ctaType: g.ctaType, count: g._count._all })),
    ...
    console.error('[GET /api/admin/whatsapp-clicks] DB error:', err);
