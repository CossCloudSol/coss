...
/*  Handler                                                                   */
/*                                                                            */
/*  Called via navigator.sendBeacon from <CallLink> — the CLIENT is          */
/*  fire-and-forget (it never waits on this response, since tel: navigation  */
/*  must never be delayed). The SERVER is not: this handler awaits the       */
/*  insert before returning, because a serverless function that returns      */
/*  before its background work finishes gets its execution frozen/killed —   */
/*  an un-awaited write here would be dropped silently on Vercel.            */
/* -------------------------------------------------------------------------- */
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  try {
    ...
    console.error('[POST /api/call-clicks] Failed to create CallClick:', err);
